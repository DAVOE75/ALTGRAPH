...
for file in files:
    if os.path.exists(file):
        ...
        # Ensure we only append once and avoid duplicating
        if "Novedades v0.6.0" not in content:
            # First fix the global versions in the file!
            content = content.replace('(v0.4.62)', '(v0.6.0 PRO)')
            
            with open(file, 'w', encoding='utf-8') as f:
                f.write(content + changelog)
